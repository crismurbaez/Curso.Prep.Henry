...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  oracion =  oracion = palabras [0];
  for (i=1;i<(palabras.length);i++) {
    oracion = oracion + " " + palabras [i];
  }
  return oracion;
  //otra forma de hacerlo es con join, investigar
...
function promedioResultadosTest(resultadosTest) {
  ...
  promedio = suma / i;
  return promedio;
  //otra forma de hacerlo, es llamando al anterior
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  num = n.toString();
   	  if (num.substr(0,1) === "9") {  //También num.charAt(0) === "9" ojo, este método sólo da un caracter
    return true;
  }
  return false;
}
...
function continueStatement(numero) {
  ...
  for (i=1;i<11;i++) {
    if (i !== 5) {
      ...
      continue;   //no entiendo que hace esto
    }
  }
  return array;
  }
